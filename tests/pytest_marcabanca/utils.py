...
class TestMachineConfiguration(TestCase):
    ...
    def test_serialize(self):
        ...
        self.assertEqual(self.serializer.deserialize(self.serializer.serialize(mc)), mc)
    ...
    def test_create_write_reference(self):
        ...
        with get_references_manager() as mngr1:
            ...
            for model_name in ["gamma", "gengamma", "norm"]:
                ...
                [
                    self.assertEqual(len(mngr1.data[_key]), 1)
                    for _key in empty_data.keys()
                ]
                self.assertEqual(
                    mngr1.data["references"][0].reference_id,
                    {
                ...
                self.assertFalse(
                    mngr1.check_reference_exists(test_node_id + "_missing")
                )
                ...
                self.assertEqual(mngr1.data, mngr2.data)
                ...
                npt.assert_array_equal(
                    ref2.model.cdf(dist_domain), ref1.model.cdf(dist_domain)
                )

                # Check rank_runtime method.
                [
                    npt.assert_array_equal(_x, _y)
                    for _x, _y in zip(
    ...
    def test_find_exact_and_approx(self):
        with get_references_manager() as mngr1:
            ...
            new_machine_config.specs["cpuinfo"][
                mdl.MachineConfiguration.cpuinfo_keys[0]
            ] += "_abc"
            ...
            pos, approx_reference = mngr1.find_approx_reference_model(
                reference_id__new_py
            )
