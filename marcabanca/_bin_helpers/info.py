...
@info.command(parents=[root_arg, diff_arg], help='Print a list of available python configurations.')
def python(root, do_diff):
    root = compute_root(root)
    mngr = Manager(root, add_this_env=False)
    print_summary(
        'python config',
        mngr.this_python_config,
        mngr.data['python_configs'],
        do_diff)
...
def print_summary(config_type, this_config, configs_list, do_diff):
    ...
    if matching_config:
        ...
        CONSOLE.print(f'Did not find a matching {config_type} for this environment!', style='red')

    # Get diff reference config
    if do_diff is None:
        ref_config = this_config
    elif do_diff is not False:
        posn = [_k for _k, _ref in enumerate(configs_list) if
                _ref.config_id[:len(do_diff)] == do_diff]
        if len(posn) > 1:
            LOGGER.error(
    ...
    formatted_configs = [
        {'this': ('*' if _config == this_config else ''),
         ...
         'specs': ref_config.diff(_config) if (do_diff is not False) else _config.for_display()}
        for _config in configs_list]

    class ColumnSpec(
            namedtuple(
                'ColumnSpec', ['key', 'header', 'kwargs'])):
        def get(self, _val):
